        ...
        <div class="content-grid">
          <section class="feed-section" aria-labelledby="moment-stream-title">
            <header class="stream-header">
              <div>
                ...
                <h2 id="moment-stream-title">最近记录</h2>
              </div>
              <div class="stream-tools">
                <PublicSelectMenu
                  ...
                  label="地点筛选"
                  empty-text="地点需要发布后才会出现在这里"
                  @update:model-value="selectPlace"
                />
                <small class="sort-indicator"
                  ><Icon name="ph:arrow-down" /> 从新到旧</small
                >
              </div>
            </header>

            <div
              v-if="!loading && !moments.length"
              class="moment-empty content-reveal"
            >
          ...
          <aside class="moments-sidebar">
            <section class="side-card overview-card">
              <header>
                <span><Icon name="ph:chart-donut" />记录概览</span
                ><small>当前加载</small>
              </header>
            ...
            <section v-if="recentIndex.length" class="side-card index-card">
              <header>
                <span><Icon name="ph:list-dashes" />这一页</span
                ><small>{{ recentIndex.length }} 条</small>
              </header>
              <button
                v-for="(moment, index) in recentIndex"
                :key="moment.slug"
                type="button"
                @click="scrollToMoment(moment.slug)"
              >
...
const visibleCommentCount = computed(() =>
  moments.value.reduce((sum, item) => sum + Number(item.commentCount || 0), 0),
);
const recentIndex = computed(() => moments.value.slice(0, 5));
const latestDate = computed(() =>
  formatSidebarDate(
    moments.value[0]?.publishedAt || moments.value[0]?.createdAt,
  ),
);
...
async function changePage() {
  await fetchMoments(page.value);
  await nextTick();
  document.querySelector<HTMLElement>(".moments-main")?.scrollTo({
    top: 0,
...
onMounted(async () => {
  const placeResult = await api
    .get<any>("/places", { limit: 100 })
    .catch(() => ({ items: [] }));
...
.moments-hero {
  ...
  box-shadow: 0 8px 30px color-mix(in srgb, var(--ld-shadow) 30%, transparent);
}
.moments-hero::before {
  position: absolute;
  top: 30px;
...
.place-filter {
  position: relative;
  width: var(--stream-control-width);
}
.place-filter > button,
.sort-indicator {
  display: flex;
  ...
  box-shadow: 0 4px 14px color-mix(in srgb, var(--ld-shadow) 22%, transparent);
  color: var(--c-text-2);
  font: inherit;
  font-size: 0.62rem;
}
...
.place-filter > button :deep(svg):first-child,
.sort-indicator :deep(svg) {
  color: var(--c-primary);
}
.place-filter > button.active {
...
.place-menu {
  ...
  box-shadow: 0 14px 32px color-mix(in srgb, var(--ld-shadow) 55%, transparent);
}
.place-menu button {
  display: flex;
  width: 100%;
...
.side-card {
  ...
  border-radius: 16px;
  background: color-mix(in srgb, var(--ld-bg-card) 96%, transparent);
  box-shadow: 0 7px 24px color-mix(in srgb, var(--ld-shadow) 30%, transparent);
}
.side-card header {
  display: flex;
  align-items: center;
...
.index-card button span {
  overflow: hidden;
  font-size: 0.65rem;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.index-card button :deep(svg) {
  color: var(--c-text-3);
  font-size: 0.7rem;
}
.note-card {
  position: relative;
  overflow: hidden;
  background: linear-gradient(145deg, var(--c-primary-soft), var(--ld-bg-card));
}
.note-card > :deep(svg) {
  color: var(--c-primary);
  font-size: 1.2rem;
...
@media (max-width: 900px) {
  .moments-main {
    padding: max(76px, calc(env(safe-area-inset-top) + 64px)) 18px 38px;
  }
  .content-grid {
