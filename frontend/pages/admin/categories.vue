<template>
  <div>
        ...
        <a-table :dataSource="categories" :columns="columns" rowKey="slug" size="small" :pagination="false" :locale="{ emptyText: '暂无分类' }">
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'icon'">
              ...
              <FolderOutlined v-else style="color:var(--c-text-3);font-size:16px" />
            </template>
    ...
    <a-modal v-model:open="addDialog.open" title="添加分类" width="460px" @ok="confirmAdd" @cancel="addDialog.open = false">
      <div class="add-field">
...
const addDialog = reactive({ open: false, name: '', slug: '', icon: 'FolderOutlined', color: '' })
...
function openAdd() {
  addDialog.name = ''; addDialog.slug = ''; addDialog.icon = 'FolderOutlined'; addDialog.color = ''
  addDialog.open = true
}
...
async function confirmAdd() {
  ...
  try {
    await api.post('/categories', { name: addDialog.name, slug, icon: addDialog.icon || null, color: addDialog.color || null })
...
async function loadViewPosts() {
  ...
  try {
    const res = await api.get<any>(`/categories/${viewDialog.slug}/posts`, { page: viewDialog.page, limit: 20 })
