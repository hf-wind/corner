      ...
      <section class="source-list" :class="{ loading }">
        ...
        <article v-for="record in pagedSources" v-else :key="record.key" class="source-row">
          <div class="source-site">
            ...
            <div>
              <strong>{{ record.name || '未命名站点' }}<em v-if="record.kind === 'friend'">自动同步</em><em v-else-if="record.origin === 'default'">默认</em></strong
              ><a :href="record.rssUrl" target="_blank" rel="noopener noreferrer">{{ record.rssUrl }}</a>
            </div>
          </div>
          <span class="source-category"><Icon :name="categoryIcon(record)" />{{ categoryLabel(record) }}</span>
          <a-switch :checked="record.enabled" size="small" @change="toggleSource(record, $event)" />
          <div class="row-actions">
            <AppLink v-if="record.kind === 'friend'" to="/admin/friends" title="前往友链管理"><Icon name="ph:arrow-square-out-bold" /></AppLink>
            <template v-else
              ><button type="button" title="编辑" @click="openEdit(record)">
                <Icon name="ph:pencil-simple-bold" /></button
              ><button type="button" class="danger" title="删除" @click="removeSource(record)">
                <Icon name="ph:trash-bold" /></button
            ></template>
          </div>
        </article>
      </section>
      ...
      <a-form layout="vertical" :model="dialog.form">
        <a-form-item label="站点地址"
          ><div class="inspect-field">
            <a-input v-model:value="dialog.form.url" placeholder="https://example.com" @input="markField('url')" @blur="inspectSite(false)" @press-enter.prevent="inspectSite(true)" /><a-button :loading="dialog.inspecting" @click="inspectSite(true)"><Icon name="ph:magic-wand-bold" /> 识别</a-button>
          </div>
          <small class="form-hint">自动识别名称、站点图标与 RSS 地址。</small>
          <div v-if="dialog.message" class="inspect-message" :class="dialog.messageType"><Icon :name="dialog.messageType === 'success' ? 'ph:check-circle-bold' : 'ph:warning-circle-bold'" />{{ dialog.message }}</div></a-form-item
        >
        ...
        <a-form-item label="前台分类" required
          ><a-select v-model:value="dialog.form.section"><a-select-option value="thought">思考</a-select-option><a-select-option value="news">新闻</a-select-option><a-select-option value="tech">科技</a-select-option><a-select-option value="ai">AI</a-select-option></a-select></a-form-item
        >
...
const sourceTabs = computed(() => [
  ...
  },
  {
    ...
    count: sources.value.filter((item) => item.kind === 'friend').length,
  },
  {
    key: 'thought',
    label: '思考',
    icon: 'ph:lightbulb-filament-bold',
    count: sources.value.filter((item) => item.kind !== 'friend' && item.section === 'thought').length,
  },
  {
    key: 'news',
    label: '新闻',
    icon: 'ph:newspaper-bold',
    count: sources.value.filter((item) => item.kind !== 'friend' && item.section === 'news').length,
  },
  {
    key: 'tech',
    label: '科技',
    icon: 'ph:cpu-bold',
    count: sources.value.filter((item) => item.kind !== 'friend' && item.section === 'tech').length,
  },
  {
    key: 'ai',
    label: 'AI',
    icon: 'ph:sparkle-bold',
    count: sources.value.filter((item) => item.kind !== 'friend' && item.section === 'ai').length,
  },
])
...
const emptySource = (): Omit<Subscription, 'key'> => ({
  name: '',
  url: '',
  rssUrl: '',
  avatar: '',
...
const dialog = reactive({
  ...
  autofilled: {} as Record<string, string>,
})

function normalize(item: any, index: number): Subscription {
  return {
    key: `source_${index}_${item.rssUrl || Date.now()}`,
    name: item.name || item.siteName || '',
    url: item.url || item.siteUrl || '',
    rssUrl: item.rssUrl || item.siteRssUrl || '',
    avatar: item.avatar || item.siteAvatar || '',
    section: item.section || 'news',
    kind: item.kind === 'friend' ? 'friend' : 'subscription',
    origin: item.origin === 'default' ? 'default' : item.origin === 'manual' ? 'manual' : undefined,
...
async function load() {
  ...
  try {
    ...
    Object.assign(config, {
      ...
      title: result?.title || '风讯角',
      subtitle: result?.subtitle || '从不同的角落，收拢值得读完的文字。',
      cacheTtl: Number(result?.cacheTtl) || 600,
      subscriptions: Array.isArray(result?.subscriptions) ? result.subscriptions.map(normalize) : [],
    })
    pagination.current = 1
  } catch (error: any) {
    toast.error(error?.message || '加载风讯角失败')
  } finally {
    loading.value = false
  }
}
function categoryLabel(item: Subscription) {
  if (item.kind === 'friend') return '友链'
  return ({ thought: '思考', news: '新闻', tech: '科技', ai: 'AI' } as Record<string, string>)[item.section || ''] || '新闻'
}
function categoryIcon(item: Subscription) {
  if (item.kind === 'friend') return 'ph:handshake-bold'
  return (
    (
      {
        thought: 'ph:lightbulb-filament-bold',
        news: 'ph:newspaper-bold',
        tech: 'ph:cpu-bold',
        ai: 'ph:sparkle-bold',
      } as Record<string, string>
    )[item.section || ''] || 'ph:newspaper-bold'
  )
}
...
function openEdit(record: Subscription) {
  dialog.editing = true
  dialog.index = sources.value.findIndex((item) => item.key === record.key)
  dialog.form = { ...record }
  dialog.touched = new Set(['name', 'url', 'rssUrl', 'avatar'])
  dialog.autofilled = {}
  dialog.message = ''
  dialog.open = true
}
function markField(field: string) {
  if (field !== 'url') dialog.touched.add(field)
}
function normalizeUrl(value: string) {
  const raw = value.trim()
  return !raw || /^[a-z][a-z\d+.-]*:\/\//i.test(raw) ? raw : `https://${raw}`
}
function onLogoError(event: Event) {
  ;(event.target as HTMLImageElement).style.display = 'none'
}
async function inspectSite(force: boolean) {
  const value = normalizeUrl(dialog.form.url)
  if (!value) {
    if (force) toast.warning('请先填写站点地址')
    return
  }
  try {
    new URL(value)
  } catch {
    if (force) toast.warning('请输入有效地址')
    return
  }
  if (dialog.inspecting) return
  dialog.form.url = value
  dialog.inspecting = true
  dialog.message = ''
  try {
    const result = await api.post<any>('/friend-link/inspect-site', {
      url: value,
    })
    const incoming = {
      name: String(result?.name || '').trim(),
      avatar: String(result?.avatar || '').trim(),
      rssUrl: String(result?.rssUrl || '').trim(),
    }
    let filled = 0
    for (const field of ['name', 'avatar', 'rssUrl'] as const) {
      const current = String(dialog.form[field] || '').trim()
      if (incoming[field] && (!current || current === dialog.autofilled[field]) && !dialog.touched.has(field)) {
        dialog.form[field] = incoming[field]
        dialog.autofilled[field] = incoming[field]
        filled++
      }
    }
    dialog.messageType = 'success'
    dialog.message = filled ? `已识别并填充 ${filled} 项` : '站点可访问，请确认 RSS 地址'
  } catch (error: any) {
    dialog.messageType = 'error'
    dialog.message = error?.message || '识别失败，请手动填写'
  } finally {
...
async function persist() {
  await api.put('/circle/admin/config', {
    ...
    subscriptions: config.subscriptions.map(({ key, ...item }) => item),
  })
}
...
async function saveSource() {
  if (!dialog.form.name.trim() || !dialog.form.rssUrl.trim()) return void toast.warning('名称和 RSS 地址不能为空')
  dialog.saving = true
  const previous = config.subscriptions.map((item) => ({ ...item }))
  const item = {
    ...dialog.form,
    kind: 'subscription' as const,
    origin: dialog.form.origin || ('manual' as const),
  }
  if (dialog.editing && dialog.index >= 0)
    config.subscriptions[dialog.index] = {
      ...item,
      key: config.subscriptions[dialog.index].key,
    }
  else config.subscriptions.push({ ...item, key: `source_${Date.now()}` })
  try {
    await persist()
    dialog.open = false
    toast.success('RSS 来源已保存')
  } catch (error: any) {
    config.subscriptions.splice(0, config.subscriptions.length, ...previous)
    toast.error(error?.message || '保存失败')
  } finally {
    dialog.saving = false
  }
}
async function toggleSource(record: Subscription, enabled: boolean) {
  const previous = record.enabled
  record.enabled = enabled
  try {
    await persist()
    toast.success(enabled ? '来源已启用' : '来源已停用')
  } catch (error: any) {
...
function removeSource(record: Subscription) {
  Modal.confirm({
    title: '移除 RSS 来源？',
    content: `将移除「${record.name}」，不会影响原站内容。`,
    okText: '移除',
    okType: 'danger',
    cancelText: '取消',
    onOk: async () => {
      const index = sources.value.findIndex((item) => item.key === record.key)
      const removed = config.subscriptions.splice(index, 1)
      try {
        await persist()
        toast.success('来源已移除')
      } catch (error: any) {
        config.subscriptions.splice(index, 0, ...removed)
...
.mode-switch button.active {
  ...
  color: var(--c-primary);
  box-shadow: 0 2px 8px color-mix(in srgb, var(--ld-shadow) 40%, transparent);
}
.config-layout {
  width: min(760px, 100%);
  margin-top: 18px;
...
.config-form :deep(.ant-form-item:nth-child(3)) {
  grid-column: 1/-1;
}
.source-filter {
  display: flex;
...
.source-filter :deep(.admin-refresh-button) {
  margin-left: auto;
}
.source-list {
  margin-top: 7px;
...
.source-row {
  display: grid;
  grid-template-columns: minmax(320px, 1fr) 100px 110px 76px;
  align-items: center;
  gap: 14px;
}
.source-list-head {
...
.source-row :deep(.ant-switch-small) {
  inline-size: 28px;
  min-inline-size: 28px;
  justify-self: start;
}
.source-row:hover {
  background: color-mix(in srgb, var(--c-primary-soft) 20%, transparent);
}
.source-site {
  display: flex;
  align-items: center;
...
.row-actions .danger:hover {
  background: color-mix(in srgb, #d85d6f 10%, transparent);
  color: #d85d6f;
}
.source-empty {
  display: flex;
