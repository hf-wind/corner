<template>
  <div class="circle-admin admin-page-shell">
    <header class="admin-page-head"><div><span>CONTENT RESOURCES</span><h1>见闻</h1><p>管理 RSS / Atom 订阅源与前台文章收录。</p></div><div class="head-actions"><template v-if="activeTab === 'config'"><a-button type="primary" :loading="saving" @click="saveConfig"><Icon name="ph:floppy-disk-bold" /> 保存配置</a-button></template><template v-else><a-button type="primary" @click="openCreate"><Icon name="ph:plus-bold" /> 添加订阅源</a-button></template></div></header>
    <a-tabs v-model:activeKey="activeTab" class="circle-tabs">
      <a-tab-pane key="config" tab="配置">
        <AdminCard icon="ph:sliders-horizontal-bold" title="见闻设置" desc="控制文章收录开关与抓取缓存。">
          <a-form layout="vertical" size="middle"><div class="form-grid"><a-form-item label="启用见闻"><a-switch v-model:checked="config.enabled" /></a-form-item><a-form-item label="缓存时间（秒）"><a-input-number v-model:value="config.cacheTtl" :min="60" :max="3600" /></a-form-item><a-form-item label="页面标题"><a-input v-model:value="config.title" placeholder="见闻" /></a-form-item><a-form-item label="页面副标题"><a-input v-model:value="config.subtitle" placeholder="从不同的角落，收拢值得读完的文字。" /></a-form-item></div></a-form>
        </AdminCard>
      </a-tab-pane>
...
type Subscription = { key: string; name: string; url: string; rssUrl: string; avatar: string; section?: string; enabled: boolean }
...
const config = reactive({ enabled: true, title: '见闻', subtitle: '从不同的角落，收拢值得读完的文字。', cacheTtl: 600, coversText: '', subscriptions: [] as Subscription[] })
...
const emptySource = (): Omit<Subscription, 'key'> => ({ name: '', url: '', rssUrl: '', avatar: '', section: undefined, enabled: true })
...
function normalize(item: any, index: number): Subscription { return { key: `source_${index}_${item.rssUrl || Date.now()}`, name: item.name || item.siteName || '', url: item.url || item.siteUrl || '', rssUrl: item.rssUrl || item.siteRssUrl || '', avatar: item.avatar || item.siteAvatar || '', section: item.section || undefined, enabled: item.enabled !== false } }
async function load() { loading.value = true; try { const result = await api.get<any>('/circle/admin/config'); Object.assign(config, { enabled: result?.enabled !== false, title: result?.title === '朋友圈' ? '见闻' : (result?.title || '见闻'), subtitle: result?.subtitle || '从不同的角落，收拢值得读完的文字。', cacheTtl: Number(result?.cacheTtl) || 600, coversText: '', subscriptions: Array.isArray(result?.subscriptions) ? result.subscriptions.map(normalize) : [] }); pagination.current = 1 } catch (error: any) { toast.error(error?.message || '加载见闻配置失败') } finally { loading.value = false } }
...
async function saveConfig() { saving.value = true; try { await persist(); toast.success('见闻配置已保存') } catch (error: any) { toast.error(error?.message || '保存配置失败') } finally { saving.value = false } }
...
onMounted(load); useHead({ title: '见闻管理' })
</script>
