  ...
  <div class="media-page">
    <div class="media-body">
      <div class="media-sidebar">
        <a-menu :selectedKeys="[activeFolder]" mode="inline" style="border-inline-end:none;background:transparent" @click="({ key }) => { if (key !== '__add__') { activeFolder = key as string; page = 1; loadMedia() } }">
          <a-menu-item key="all"><FolderOutlined /> 全部文件</a-menu-item>
          <a-menu-item key="__none__"><InboxOutlined /> 未分类</a-menu-item>
          <a-menu-item v-for="f in folders" :key="f"><FolderFilled /> {{ f }}</a-menu-item>
          <a-menu-item key="__add__" class="menu-add-folder" @click="showNewFolder = true">
          ...
          <a-card :bordered="false" class="list-card" size="small">
            <div class="media-grid" v-if="items.length">
              <div v-for="item in items" :key="item.id" class="media-item">
                <div v-if="isImage(item)" class="media-img-wrap"><a-image :src="imgUrl(item)" style="width:100%;height:120px;object-fit:cover" /></div>
    ...
    <a-modal v-model:open="showNewFolder" title="新建文件夹" @ok="createFolder" @cancel="showNewFolder = false">
      <a-input v-model:value="newFolderName" placeholder="文件夹名称" />
    </a-modal>
  </div>
</template>
...
const page = ref(1); const limit = 30
...
async function loadMedia() {
  ...
  } catch { items.value = []; total.value = 0; totalPages.value = 0 }
  loading.value = false
}

async function loadFolders() {
  try {
    const res = await api.get<string[]>('/media/folders')
...
function beforeUpload(file: File) {
  ...
  if (activeFolder.value !== 'all' && activeFolder.value !== '__none__') {
    fd.append('folder', activeFolder.value)
  }
  api.upload('/media/upload', fd).then(() => { message.success('上传成功'); loadMedia(); loadFolders() }).catch(() => message.error('上传失败'))
  return false
}
...
async function handleRemove(item: any) {
  Modal.confirm({
    ...
    cancelText: '取消',
    onOk: async () => {
      try { await api.delete(`/media/${item.id}`); message.success('已删除'); loadMedia(); loadFolders() }
...
async function createFolder() {
  ...
  try {
    ...
    message.success('已创建'); showNewFolder.value = false; newFolderName.value = ''; loadFolders()
...
.media-sidebar { width:180px; flex-shrink:0; }
