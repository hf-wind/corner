<template>
  <div class="profile-page admin-page-shell">
        ...
        <div class="profile-head">
          <div class="avatar-box">
            ...
            <label class="avatar-upload" :class="{ busy: avatarUploading }">
              ...
              <Icon
                :name="
                  avatarUploading ? 'ph:spinner-gap-bold' : 'ph:camera-bold'
                "
              />
            </label>
          </div>
          <div class="head-meta">
            ...
            <div class="avatar-hint">
              JPG、PNG 或 WebP，最大 8MB；上传后自动压缩并归入头像文件夹。
            </div>
        ...
        >
          ...
          <a-form-item>
            <a-button type="primary" html-type="submit" :loading="saving"
              ><Icon name="ph:floppy-disk-bold" /> 保存修改</a-button
            >
          </a-form-item>
        </a-form>
      </a-card>
        ...
        >
          ...
          >
            <div class="code-row">
              ...
              <a-button
                ...
                @click="passwordChallengeOpen = true"
              >
          ...
          <a-form-item>
            <a-button type="primary" html-type="submit" :loading="pwdSaving"
              ><Icon name="ph:key-bold" /> 修改密码</a-button
            >
          </a-form-item>
        </a-form>
      </a-card>
    ...
    <AuthChallengeModal
      :open="passwordChallengeOpen"
      :busy="passwordChallengeBusy"
      title="验证账号安全"
      description="完成校验后，我们会发送设置密码所需的验证码。"
      @cancel="passwordChallengeOpen = false"
      @verified="sendChangePasswordCode"
    />
  </div>
</template>
...
const { user, isAdmin, setSession, token, readStorage, refreshProfile } =
  useAuth();
...
const passwordChallengeOpen = ref(false);
const passwordChallengeBusy = ref(false);
...
async function sendChangePasswordCode(geetestToken: string) {
  ...
  passwordChallengeBusy.value = true;
  try {
    ...
    passwordChallengeBusy.value = false;
  }
}
