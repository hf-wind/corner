        ...
        <a-spin :spinning="musicLoading">
          ...
          >
            ...
            <a-form-item label="默认 ID">
              <a-input
                v-model:value="music.music_id"
                placeholder="歌单/专辑/歌曲 ID"
              />
            </a-form-item>
            ...
            <a-form-item label="歌单列表">
              <div class="playlist-editor">
                ...
                <a-button size="small" @click="addPlaylist"
                  >+ 添加歌单</a-button
                >
              </div>
            </a-form-item>
  ...
  try {
    ...
    await api.put("/music/admin/config", {
      config: {
        ...
        music_playlists: music.music_playlists
          .filter((p) => p.id?.trim())
          .map(({ key: _key, ...playlist }) => playlist),
      },
    });
