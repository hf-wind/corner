  ...
  <div class="page-layout article-page">
    <main id="main-content" ref="articleMainRef" class="article-main" :class="{ 'article-ready': articleReady }"
      ...
      <div class="post-header article-anim" :class="{ 'has-cover': article.hero }">
        ...
        <div class="post-nav">
          <div class="operations">
            <button type="button" class="z-btn" title="文字分享" @click="shareOpen = true">
              <Icon name="ph:share-bold" />
              <span>文字分享</span>
            </button>
            <button type="button" class="z-btn" title="生成分享海报" @click="posterOpen = true">
      ...
      <div v-if="adjacentLoaded" class="surround-post article-anim">
        ...
        <NuxtLink v-if="nextArticle" :to="'/article/' + nextArticle.slug" class="surround-link align-end">
...
function checkOutdated() {
  ...
  const daysPassed = Math.floor((Date.now() - publishTime) / (1000 * 60 * 60 * 24))
...
.surround-link:hover {
  ...
  box-shadow: 0 12px 28px color-mix(in srgb, var(--c-primary) 12%, var(--ld-shadow));
}
