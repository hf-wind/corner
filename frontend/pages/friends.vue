  ...
  <div class="page-layout">
    <main class="main-content">
      ...
      <section class="friend-welcome">
        <div class="welcome-orbit" aria-hidden="true"><i /><i /><Icon name="ph:planet-duotone" /></div>
      ...
      <Transition name="form-reveal">
        <section v-if="showApplyForm" id="friend-apply-form" class="form-card">
          ...
          <p class="form-intro">欢迎交换友链。提交前，请先把本站添加到你的友链页面。</p>
          <a-form :model="applyForm" layout="vertical" class="apply-form" @finish="submitApply">
            ...
            <a-form-item label="联系邮箱" required><a-input v-model:value="applyForm.contactEmail" placeholder="用于接收审核结果" /></a-form-item>
      ...
      <div v-if="loading" class="friends-grid"><div v-for="n in 6" :key="n" class="friend-skeleton"><i /><strong /><span /><span /></div></div>
      ...
      <div v-else class="empty-state"><Icon name="ph:handshake" /><strong>还没有网络邻居</strong><span>第一条友链会让这里热闹起来。</span></div>
    </main>
...
.friend-skeleton { display:flex; min-height:214px; flex-direction:column; align-items:center; padding:24px; border:1px solid var(--border); border-radius:16px; background:var(--ld-bg-card); }.friend-skeleton i,.friend-skeleton strong,.friend-skeleton span { display:block; border-radius:999px; background:linear-gradient(90deg,var(--c-bg-2),var(--ld-bg-card),var(--c-bg-2)); background-size:200% 100%; animation:shimmer 1.4s infinite; }.friend-skeleton i { width:62px; height:62px; }.friend-skeleton strong { width:55%; height:12px; margin-top:16px; }.friend-skeleton span { width:90%; height:8px; margin-top:10px; }.friend-skeleton span:last-child { width:68%; }
...
@media (max-width:640px) { .main-content { padding:max(68px,calc(env(safe-area-inset-top) + 60px)) 16px 24px!important; }.friend-welcome { grid-template-columns:1fr; margin-inline:0; padding:20px; }.welcome-orbit { display:none; }.welcome-steps { flex-wrap:wrap; }.welcome-actions { grid-column:auto; }.friends-grid { gap:10px; }.friend-card { min-height:208px; padding-inline:12px; }.apply-form { grid-template-columns:1fr; }.span-two { grid-column:auto; }.form-actions { align-items:flex-start; flex-direction:column; }.form-intro { margin-left:0; }.content-heading { align-items:flex-start; flex-direction:column; gap:4px; } }
