...
const title = ref("正在处理退订…");
const message = ref("请稍候，我们正在处理你的请求。");

const iconName = computed(() => {
  if (state.value === "success") return "ph:bell-slash-bold";
...
onMounted(async () => {
  ...
  if (!token) {
    state.value = "error";
    title.value = "链接无效";
    message.value = "退订链接缺失或格式不正确，请从邮件里重新点击。";
    ready.value = true;
    return;
  }
  try {
    await api.post<any>("/newsletter/unsubscribe", { token });
    state.value = "success";
    title.value = "已退订";
    message.value =
      "你不会再收到风隅周报的邮件。如果改变主意，随时欢迎回来重新订阅。";
  } catch (error: any) {
    state.value = "error";
    title.value = "退订失败";
    message.value = error?.message || "链接可能已失效，请重新操作。";
  } finally {
    ready.value = true;
  }
});

useHead({ title: "退订周报 · 风隅随笔" });
</script>
