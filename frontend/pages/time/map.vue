  ...
  <main class="memory-map-page">
    ...
    <aside class="memory-panel" :class="{ hidden: mobileMode === 'map' }">
      ...
      <div ref="listEl" class="memory-list">
        ...
        <article v-if="item.kind === 'memory'" :id="`map-memory-${safeId(item.id)}`" :class="['memory-card', item.type, { active: selectedId === item.id }]" @mouseenter="selectMemory(item, false)" @click="selectMemory(item, false, true)">
...
.memory-panel { position: absolute; z-index: 20; top: 18px; right: 18px; bottom: 18px; width: min(390px, calc(100% - 36px)); border: 1px solid color-mix(in srgb, var(--border) 72%, transparent); border-radius: 18px; background: color-mix(in srgb, var(--c-bg) 86%, transparent); box-shadow: 0 18px 48px rgb(22 30 45 / 20%); backdrop-filter: blur(18px); }
.memory-panel .panel-head { padding: 22px 20px 16px; }
.memory-panel .filters { padding-inline: 20px; }
.memory-panel .panel-meta { padding-inline: 20px; }
.memory-panel .memory-list { padding-inline: 12px; }
