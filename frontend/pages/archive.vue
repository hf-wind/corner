<template>
  <div class="page-layout">
    <main class="main-content">
      ...
      <div v-else-if="!loading" class="empty-state content-reveal"><Icon name="ph:archive-box" /><strong>时间轴还没有内容</strong><span>第一篇文章会成为这里的起点。</span></div>
    </main>
...
.right-card-title>span > .icon { width:26px; height:26px; border:1px solid color-mix(in srgb,var(--c-primary) 12%,transparent); border-radius:8px; background:linear-gradient(145deg,var(--c-primary-soft),color-mix(in srgb,var(--ld-bg-card) 72%,transparent)); color:var(--c-primary); place-items:center; }
</style>
