<template>
  <div class="page-layout">
    <main class="main-content">
      ...
      <section class="category-insights" aria-label="分类概况">
        <div>
          <span><Icon name="ph:article-medium-bold" /></span>
          <p><strong>{{ totalCategoryPosts }}</strong><small>已归档文章</small></p>
        </div>
        <i />
        <div>
          <span><Icon name="ph:crown-simple-bold" /></span>
          <p><strong>{{ leadingCategory?.name || '等待内容' }}</strong><small>内容最多的分类</small></p>
        </div>
        <i />
        <div>
          <span><Icon name="ph:compass-tool-bold" /></span>
          <p><strong>{{ activeCategoryName || '全部内容' }}</strong><small>当前阅读范围</small></p>
        </div>
      </section>

      <div v-if="loading" class="category-grid skeleton-grid">
        <div v-for="n in 4" :key="n" class="category-skeleton"><i /><div><strong /><span /><span /></div></div>
      </div>
      <template v-else>
        ...
        <section class="result-section">
          ...
          <div v-else-if="categoryPosts.length" class="article-list">
            <CompactArticleCard v-for="article in categoryPosts" :key="article.slug" :article="article" />
          </div>
          <div v-else class="empty-state"><Icon name="ph:tray" /><strong>{{ activeCategory ? '这个分类暂时没有文章' : '还没有已发布文章' }}</strong><span>{{ activeCategory ? '换一个分类看看吧。' : '新的内容会在这里出现。' }}</span></div>
        </section>
      </template>
    </main>
...
.category-insights { display:grid; grid-template-columns:minmax(0,1fr) 1px minmax(0,1.35fr) 1px minmax(0,1.25fr); align-items:center; gap:18px; margin:-9px 18px 28px; padding:15px 20px; border:1px solid color-mix(in srgb,var(--border) 70%,transparent); border-radius:0 0 15px 15px; background:color-mix(in srgb,var(--ld-bg-card) 88%,transparent); box-shadow:0 10px 24px color-mix(in srgb,var(--ld-shadow) 24%,transparent); backdrop-filter:blur(16px); }.category-insights>i { width:1px; height:29px; background:var(--border); }.category-insights>div { display:flex; min-width:0; align-items:center; gap:10px; }.category-insights>div>span { display:grid; width:34px; height:34px; flex:0 0 34px; border-radius:10px; background:linear-gradient(145deg,var(--c-primary-soft),color-mix(in srgb,var(--ld-bg-card) 70%,transparent)); color:var(--c-primary); font-size:1rem; place-items:center; }.category-insights p { display:flex; min-width:0; flex-direction:column; gap:2px; margin:0; }.category-insights strong { overflow:hidden; color:var(--c-text); font-size:.7rem; text-overflow:ellipsis; white-space:nowrap; }.category-insights small { color:var(--c-text-3); font-size:.5rem; letter-spacing:.05em; }
...
.skeleton-grid { margin-top:4px; }.category-skeleton { display:grid; min-height:132px; grid-template-columns:48px 1fr; align-items:center; gap:14px; padding:20px; border:1px solid var(--border); border-radius:16px; background:var(--ld-bg-card); }.category-skeleton i,.category-skeleton strong,.category-skeleton span,.article-skeletons i { display:block; border-radius:8px; background:linear-gradient(90deg,var(--c-bg-2),var(--ld-bg-card),var(--c-bg-2)); background-size:200% 100%; animation:shimmer 1.4s infinite; }.category-skeleton i { width:48px; height:48px; }.category-skeleton strong { width:45%; height:14px; }.category-skeleton span { height:8px; margin-top:10px; }.article-skeletons { display:grid; gap:10px; }.article-skeletons i { height:104px; }
