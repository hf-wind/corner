  ...
  <main class="album-detail">
    <template v-if="album">
      ...
      <ImageLightbox v-model="lightboxOpen" v-model:index="lightboxIndex" :images="lightboxImages" label="相册照片">
        <template #caption="{ image, index }"><div class="lightbox-rich-caption"><p>{{ image.caption }}</p><div v-if="album.items?.[index]?.moment || album.items?.[index]?.publicLocation"><NuxtLink v-if="album.items[index].moment" :to="`/moments?focus=${album.items[index].moment.slug}`"><Icon name="ph:sparkle-bold" />{{ album.items[index].moment.title }}</NuxtLink><span v-if="album.items[index].publicLocation"><Icon name="ph:map-pin-fill" />{{ album.items[index].publicLocation.name }}</span></div></div></template>
      </ImageLightbox>
    </template>
