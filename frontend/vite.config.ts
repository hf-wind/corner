  ...
  return {
    ...
    load(id) {
      ...
      for (const file of files) {
        ...
        for (const match of source.matchAll(/(?:ph|solar):[a-z0-9-]+/g)) names.add(match[0])
      }

      const collections: Record<string, IconifyJSON> = {
        ph: JSON.parse(readFileSync(resolve(root, 'node_modules/@iconify-json/ph/icons.json'), 'utf8')),
        solar: JSON.parse(readFileSync(resolve(root, 'node_modules/@iconify-json/solar/icons.json'), 'utf8')),
      }
...
function autoComponentsPlugin(): Plugin {
  ...
  return {
    ...
    transform(source, id) {
      ...
      for (const match of source.matchAll(/<([A-Z][A-Za-z0-9]*|[a-z][a-z0-9]*(?:-[a-z0-9]+)+)\b/g)) {
        const tag = match[1]
        ...
        const hasBinding = [
          ...
          new RegExp(`\\bimport\\s*\\{[^}]*\\b(?:${localName}|\\w+\\s+as\\s+${localName})\\b[^}]*\\}`, 's'),
          new RegExp(`\\b(?:const|let|var|function|class)\\s+${localName}\\b`),
          new RegExp(`\\b(?:const|let|var)\\s*\\{[^}]*\\b${localName}\\b[^}]*\\}`, 's'),
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      ...
      Unimport.vite({
        imports: [
          ...['computed', 'nextTick', 'onBeforeUnmount', 'onMounted', 'onUnmounted', 'reactive', 'readonly', 'ref', 'shallowRef', 'unref', 'useId', 'watch'].map((name) => ({ name, from: 'vue' })),
          ...['useRoute', 'useRouter'].map((name) => ({ name, from: 'vue-router' })),
          { name: 'useHead', from: '@unhead/vue' },
          ...['useState', 'useRuntimeConfig', 'navigateTo', 'definePageMeta'].map((name) => ({ name, from: '~/compat/runtime' })),
        ],
