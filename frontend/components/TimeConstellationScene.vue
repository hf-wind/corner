...
let stationOrbitPhase = -.72
let satelliteOrbitPhase = -2.28
let spacecraftCruisePhase = -.56
...
function corePlanetTexture() {
  ...
  const landColors = ['rgba(73, 139, 122, .78)', 'rgba(91, 154, 124, .7)', 'rgba(65, 111, 105, .74)']
  ...
  const southIce = context.createLinearGradient(0, canvas.height - 42, 0, canvas.height)
  ...
  for (const x of [96, 288, 480, 672]) drawLogoMark(context, x, 192, 1.55, .32)
...
function coreCloudTexture() {
  ...
  for (let system = 0; system < (lowQuality ? 38 : 72); system++) {
    ...
    for (let puff = 0; puff < 5; puff++) {
      ...
      context.fillStyle = `rgba(238, 249, 255, ${.08 + random() * .28})`
      context.fill()
    }
    context.restore()
  }
...
function accretionDiskTexture() {
  ...
  for (let index = 0; index < 360; index++) {
    ...
    context.strokeStyle = hot
      ? `rgba(255, ${172 + Math.floor(random() * 72)}, ${72 + Math.floor(random() * 95)}, ${.12 + random() * .58})`
      : `rgba(210, ${76 + Math.floor(random() * 80)}, ${28 + Math.floor(random() * 52)}, ${.04 + random() * .24})`
    context.stroke()
  }
  ...
  veil.addColorStop(.48, 'rgba(229,91,32,.16)')
...
function addStarField() {
  ...
  const layerSettings = lowQuality
    ? [
        { count: 920, near: 125, depth: 430, size: 1.45, opacity: .9, color: 0xd9efff },
        { count: 90, near: 80, depth: 240, size: 3.2, opacity: .72, color: 0xffe3b0 },
      ]
    : [
        { count: 2400, near: 125, depth: 520, size: 1.26, opacity: .9, color: 0xd9efff },
        { count: 980, near: 72, depth: 320, size: 2.05, opacity: .68, color: 0x87baff },
        { count: 180, near: 58, depth: 240, size: 3.8, opacity: .7, color: 0xffdfaa },
      ]
  ...
  for (let index = 0; index < dustCount; index++) {
    const radius = 120 + Math.pow(random(), .72) * 430
    const theta = random() * Math.PI * 2
    const vertical = (random() - .5) * (18 + radius * .08)
    dustPositions[index * 3] = Math.cos(theta) * radius
    dustPositions[index * 3 + 1] = vertical
    dustPositions[index * 3 + 2] = Math.sin(theta) * radius
    const shade = coolDust.clone().lerp(warmDust, random() * .7)
...
function addCore() {
  ...
  const orbGeometry = track(new THREE.SphereGeometry(10.2, lowQuality ? 24 : 64, lowQuality ? 16 : 40))
  const orb = new THREE.Mesh(orbGeometry, orbMaterial)
  orb.name = 'core-surface'
  core.add(orb)
  const clouds = new THREE.Mesh(orbGeometry, track(new THREE.MeshStandardMaterial({
    color: 0xffffff,
  ...
  clouds.rotation.y = .34
  core.add(clouds)
  const coreAtmosphere = new THREE.Mesh(orbGeometry, atmosphereMaterial(0x69caff, 1.08))
  ...
  const planetaryGlow = new THREE.Sprite(track(new THREE.SpriteMaterial({
  ...
  for (const [scale, tilt] of [[18, .9], [24, -1.15]] as const) {
  ...
  moonOrbit.rotation.set(.42, 0, -.16)
  const moon = new THREE.Mesh(
    track(new THREE.SphereGeometry(1.25, lowQuality ? 12 : 24, lowQuality ? 8 : 16)),
    track(new THREE.MeshStandardMaterial({ color: 0xaeb8c0, roughness: .9, metalness: .02, emissive: 0x18222b, emissiveIntensity: .18 })),
  )
...
function addBlackHole() {
  ...
  const horizontalOffset = window.innerWidth < 720 ? -.18 : -.46
  blackHole.userData.viewOffset = new THREE.Vector3(horizontalOffset, .22, -1).normalize()
  blackHole.position.set(-112, 64, -164)
  blackHole.rotation.set(.04, -.16, -.08)
  blackHole.scale.setScalar(1.12)
  const eventHorizon = new THREE.Mesh(
    track(new THREE.SphereGeometry(13.6, lowQuality ? 24 : 48, lowQuality ? 16 : 32)),
  ...
  const diskMaterial = track(new THREE.MeshBasicMaterial({ map: diskTexture, color: 0xffffff, transparent: true, opacity: .95, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide, fog: false }))
  for (const [name, inner, outer, tilt, opacity] of [
    ['black-hole-disk', 14.4, 49, 1.34, .98],
    ['black-hole-lensed-disk', 15.2, 42, -.58, .42],
  ] as const) {
    const layerMaterial = name === 'black-hole-disk' ? diskMaterial : track(diskMaterial.clone())
    layerMaterial.opacity = opacity
    const layer = new THREE.Mesh(track(new THREE.RingGeometry(inner, outer, lowQuality ? 96 : 220, 1)), layerMaterial)
    layer.name = name
    layer.rotation.set(tilt, .06, -.18)
    layer.renderOrder = name === 'black-hole-disk' ? 10 : 6
    blackHole.add(layer)
  }

  const photonRing = new THREE.Mesh(
    track(new THREE.TorusGeometry(14.7, .28, 10, lowQuality ? 96 : 220)),
    track(new THREE.MeshBasicMaterial({ color: 0xffbd72, transparent: true, opacity: .78, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide, fog: false })),
  )
  photonRing.name = 'black-hole-photon-ring'
  photonRing.rotation.set(.12, .08, -.18)
  photonRing.renderOrder = 14
  blackHole.add(photonRing)

  const crescentMaterial = track(new THREE.MeshBasicMaterial({ color: 0xff9a42, transparent: true, opacity: .78, blending: THREE.AdditiveBlending, depthWrite: false, fog: false }))
  for (const [radius, tube, arc, rotationZ, opacity] of [[17.2, .75, Math.PI * 1.42, -.82, .92], [21.4, .28, Math.PI * 1.18, 2.46, .48]] as const) {
    const material = track(crescentMaterial.clone()); material.opacity = opacity
    const crescent = new THREE.Mesh(track(new THREE.TorusGeometry(radius, tube, 8, lowQuality ? 72 : 150, arc)), material)
    crescent.name = 'black-hole-crescent'
    crescent.rotation.set(.1, .04, rotationZ)
    crescent.renderOrder = 15
    blackHole.add(crescent)
  }

  const halo = new THREE.Sprite(track(new THREE.SpriteMaterial({ map: glowTexture('#ff5d24'), transparent: true, opacity: .22, depthWrite: false, fog: false, blending: THREE.AdditiveBlending })))
...
function addSpaceStation() {
  ...
  spaceStation.rotation.set(.18, -stationOrbitPhase + Math.PI / 2, -.12)
  ...
  for (let index = 0; index <= 240; index++) orbitPoints.push(stationOrbitPosition(index / 240 * Math.PI * 2))
  const orbit = new THREE.Line(
    track(new THREE.BufferGeometry().setFromPoints(orbitPoints)),
    track(new THREE.LineBasicMaterial({ color: 0x4f9fc8, transparent: true, opacity: .14, blending: THREE.AdditiveBlending, depthWrite: false })),
  )
...
function addSpacecraft() {
  ...
  const ceramic = track(new THREE.MeshStandardMaterial({ color: 0xd9e2e8, roughness: .2, metalness: .82, emissive: 0x10263a, emissiveIntensity: .22 }))
  const titanium = track(new THREE.MeshStandardMaterial({ color: 0x31485a, roughness: .3, metalness: .9, emissive: 0x07131d, emissiveIntensity: .3 }))
  const carbon = track(new THREE.MeshStandardMaterial({ color: 0x101c26, roughness: .5, metalness: .7 }))
  const armor = track(new THREE.MeshStandardMaterial({ color: 0x8397a6, roughness: .24, metalness: .88, emissive: 0x0b2334, emissiveIntensity: .28 }))
  const accent = track(new THREE.MeshStandardMaterial({ color: 0x87dff7, roughness: .14, metalness: .72, emissive: 0x1687b5, emissiveIntensity: 1.15 }))
  const canopy = track(new THREE.MeshStandardMaterial({ color: 0x4dbbe8, roughness: .08, metalness: .68, emissive: 0x1475a4, emissiveIntensity: .82, transparent: true, opacity: .88 }))
  const engineGlow = track(new THREE.SpriteMaterial({ map: glowTexture('#63dcff'), color: 0x85e9ff, transparent: true, opacity: .94, blending: THREE.AdditiveBlending, depthWrite: false }))
  const navigationGlow = track(new THREE.MeshBasicMaterial({ color: 0x78e9ff, transparent: true, opacity: .9, blending: THREE.AdditiveBlending, depthWrite: false }))
  const shipLight = new THREE.PointLight(0x9bdfff, 42, 52, 1.6)
  shipLight.position.set(3, 8, 10)
  spacecraft.add(shipLight)

  const spine = new THREE.Mesh(track(new THREE.CapsuleGeometry(2.45, 11.5, lowQuality ? 5 : 9, lowQuality ? 10 : 20)), ceramic)
  spine.rotation.z = -Math.PI / 2
  spacecraft.add(spine)
  const aftBody = new THREE.Mesh(track(new THREE.CylinderGeometry(3.05, 3.7, 6.2, lowQuality ? 12 : 24)), titanium)
  aftBody.position.x = -6.1
  aftBody.rotation.z = Math.PI / 2
  spacecraft.add(aftBody)
  const nose = new THREE.Mesh(track(new THREE.ConeGeometry(2.38, 7.2, lowQuality ? 12 : 28)), ceramic)
  nose.position.x = 10.8
  nose.rotation.z = -Math.PI / 2
  spacecraft.add(nose)
  const noseBlade = new THREE.Mesh(track(new THREE.ConeGeometry(.42, 3.6, lowQuality ? 8 : 16)), accent)
  noseBlade.position.x = 15.3
  noseBlade.rotation.z = -Math.PI / 2
  spacecraft.add(noseBlade)
  const keel = new THREE.Mesh(track(new THREE.BoxGeometry(13.8, .72, 1.2)), titanium)
  keel.position.set(-.5, -2.15, 0)
  spacecraft.add(keel)

  const cockpitFrame = new THREE.Mesh(track(new THREE.SphereGeometry(2.58, lowQuality ? 14 : 28, lowQuality ? 8 : 16)), titanium)
  cockpitFrame.position.set(4.8, 1.2, 0)
  cockpitFrame.scale.set(1.38, .57, .82)
  spacecraft.add(cockpitFrame)
  const cockpit = new THREE.Mesh(track(new THREE.SphereGeometry(2.4, lowQuality ? 14 : 28, lowQuality ? 8 : 16, 0, Math.PI * 2, 0, Math.PI * .72)), canopy)
  cockpit.position.set(4.9, 1.25, 0)
  cockpit.scale.set(1.35, .54, .78)
  cockpit.rotation.z = -.08
  spacecraft.add(cockpit)
  for (const x of [2.8, 4.5, 6.2]) {
    const canopyRib = new THREE.Mesh(track(new THREE.BoxGeometry(.12, .2, 4)), accent)
    canopyRib.position.set(x, 1.55, 0)
    canopyRib.rotation.z = -.08
    spacecraft.add(canopyRib)
  }

  for (const side of [-1, 1]) {
    const shoulder = new THREE.Mesh(track(new THREE.BoxGeometry(8.4, .48, 2.3)), armor)
    shoulder.position.set(-.5, .15, side * 2.5)
    shoulder.rotation.y = side * -.09
    spacecraft.add(shoulder)
    const shoulderLight = new THREE.Mesh(track(new THREE.BoxGeometry(6.6, .12, .12)), accent)
    shoulderLight.position.set(.1, .45, side * 3.7)
    shoulderLight.rotation.y = side * -.09
  ...
  wingGeometry.setAttribute('position', new THREE.Float32BufferAttribute([
    3, 0, 0, -4.5, 0, 0, -8.5, 0, 8.8,
    3, -.38, 0, -4.5, -.38, 0, -8.5, -.38, 8.8,
  ], 3))
  wingGeometry.setIndex([0, 1, 2, 5, 4, 3, 0, 3, 4, 0, 4, 1, 1, 4, 5, 1, 5, 2, 2, 5, 3, 2, 3, 0])
  ...
  for (const side of [-1, 1]) {
    ...
    wing.position.y = -.3
    spacecraft.add(wing)
    const edge = new THREE.Mesh(track(new THREE.BoxGeometry(9.8, .16, .26)), navigationGlow)
    edge.position.set(-3.1, -.25, side * 5.2)
    edge.rotation.y = side * -.49
    spacecraft.add(edge)
    const wingArmor = new THREE.Mesh(track(new THREE.BoxGeometry(7.6, .3, 2.7)), armor)
    wingArmor.position.set(-3.1, -.08, side * 5.25)
    wingArmor.rotation.y = side * -.34
    wingArmor.scale.z = .72
    spacecraft.add(wingArmor)
    const wingTip = new THREE.Mesh(track(new THREE.CapsuleGeometry(.48, 3.4, lowQuality ? 3 : 6, lowQuality ? 8 : 14)), titanium)
    wingTip.position.set(-6.9, -.2, side * 8.25)
    wingTip.rotation.z = Math.PI / 2
    spacecraft.add(wingTip)
    const wingBeacon = new THREE.Sprite(track(new THREE.SpriteMaterial({ map: glowTexture(side < 0 ? '#ff657b' : '#69e7ff'), transparent: true, opacity: .86, blending: THREE.AdditiveBlending, depthWrite: false })))
  ...
  for (const side of [-1, 1]) {
    const tail = new THREE.Mesh(track(new THREE.ConeGeometry(1.05, 5.2, 3)), ceramic)
    tail.position.set(-7.4, 2.7, side * 2.35)
    tail.rotation.set(.08, 0, side * -.24)
    tail.scale.set(.5, 1, .82)
    spacecraft.add(tail)
  }

  const reactorRing = new THREE.Mesh(track(new THREE.TorusGeometry(2.72, .18, lowQuality ? 8 : 12, lowQuality ? 28 : 56)), accent)
  reactorRing.position.x = -3.45
  reactorRing.rotation.y = Math.PI / 2
  spacecraft.add(reactorRing)

  for (const z of [-2.15, 0, 2.15]) {
    const nacelle = new THREE.Mesh(track(new THREE.CylinderGeometry(1.02, 1.35, 4.1, lowQuality ? 12 : 24)), titanium)
    nacelle.position.set(-8.1, 0, z)
    nacelle.rotation.z = Math.PI / 2
    spacecraft.add(nacelle)
    const nacelleArmor = new THREE.Mesh(track(new THREE.CylinderGeometry(1.12, 1.12, 1.5, lowQuality ? 12 : 24)), armor)
    nacelleArmor.position.set(-7.15, 0, z)
    nacelleArmor.rotation.z = Math.PI / 2
    spacecraft.add(nacelleArmor)
    const ring = new THREE.Mesh(track(new THREE.TorusGeometry(1.02, .16, 8, lowQuality ? 24 : 48)), navigationGlow)
  ...
  for (const [engineIndex, z] of [-2.15, 0, 2.15].entries()) {
    ...
    const hotCore = new THREE.Mesh(
      track(new THREE.ConeGeometry(.62, 11.5, lowQuality ? 10 : 18, 1, true)),
      enginePlumeMaterial(true, engineIndex * 1.73 + .8),
    )
    hotCore.name = 'spacecraft-engine-plume'
    hotCore.position.set(-15.9, 0, z)
    hotCore.rotation.z = Math.PI / 2
    hotCore.userData.baseLength = .72
    ...
    for (let index = 0; index < particleCount; index++) {
      ...
      particlePositions[index * 3 + 1] = (random() - .5) * (1 + random() * 1.4)
      particlePositions[index * 3 + 2] = z + (random() - .5) * (1 + random() * 1.2)
      particleSpeeds[index] = 11 + random() * 19
    }
    const particleGeometry = track(new THREE.BufferGeometry())
    particleGeometry.setAttribute('position', new THREE.BufferAttribute(particlePositions, 3))
    const particles = new THREE.Points(particleGeometry, track(new THREE.PointsMaterial({
      color: engineIndex === 1 ? 0xffe6b8 : 0x83dcff,
      map: glowTexture(engineIndex === 1 ? '#ffe6b8' : '#76cfff'),
      size: lowQuality ? .46 : .62,
      transparent: true,
      opacity: .64,
      alphaTest: .03,
      depthWrite: false,
  ...
  const trailGeometry = track(new THREE.BufferGeometry().setFromPoints([
    new THREE.Vector3(-5, -.3, -7), new THREE.Vector3(-38, -.3, -9.5),
    new THREE.Vector3(-5, -.3, 7), new THREE.Vector3(-38, -.3, 9.5),
  ]))
  const sideTrails = new THREE.LineSegments(trailGeometry, track(new THREE.LineBasicMaterial({ color: 0x78dfff, transparent: true, opacity: .12, blending: THREE.AdditiveBlending, depthWrite: false })))
  sideTrails.name = 'spacecraft-ion-trails'
  spacecraft.add(sideTrails)

  const markingCanvas = document.createElement('canvas')
  markingCanvas.width = 384; markingCanvas.height = 96
  const markingContext = markingCanvas.getContext('2d')!
  markingContext.fillStyle = 'rgba(6,18,28,.86)'; markingContext.fillRect(0, 0, 384, 96)
  markingContext.fillStyle = '#9de8ff'; markingContext.font = '700 42px sans-serif'; markingContext.textAlign = 'center'; markingContext.textBaseline = 'middle'
  markingContext.fillText('风隅号  FY-01', 192, 48)
  const markingTexture = track(new THREE.CanvasTexture(markingCanvas)); markingTexture.colorSpace = THREE.SRGBColorSpace
  const marking = new THREE.Mesh(track(new THREE.PlaneGeometry(7.4, 1.85)), track(new THREE.MeshBasicMaterial({ map: markingTexture, transparent: true, opacity: .88, side: THREE.DoubleSide })))
  marking.position.set(-.5, .2, 2.46)
...
function updateFocusedDiscoveryCamera(delta: number) {
  if (!camera || !controls || cameraFlight || discoveryTourId || !focusedDiscoveryId) return
  if (focusedDiscoveryId !== 'spacecraft' && focusedDiscoveryId !== 'station') return
  const object = discoveryObjects.get(focusedDiscoveryId)
  if (!object) return
  const target = object.getWorldPosition(new THREE.Vector3())
...
function applySelection(animate = true) {
  ...
  for (const [id, object] of nodeObjects) {
    ...
    const targetScale = id === selected ? 1.34 : selected ? .86 : 1
    object.userData.selectionScale = animate ? Number(object.userData.selectionScale || object.scale.x || 1) : targetScale
    ...
    object.traverse((child) => {
      ...
      if ('opacity' in material && Number.isFinite(baseOpacity)) {
        ...
        const targetOpacity = Math.min(1, baseOpacity * (active ? boost : .2))
      ...
      if (Number.isFinite(baseGlow) && material.uniforms?.glowOpacity) {
        const targetGlow = baseGlow * (id === selected ? 1.42 : active ? 1 : .22)
...
function applyCruiseCamera(now: number, delta: number) {
  if (!camera || !controls || !introCompleted || cameraFlight || discoveryTourId || props.selectedId || focusedDiscoveryId) return
  if (!props.ambient && now < cruisePausedUntil) return
  ambientOrbitPhase -= delta * CRUISE_ANGULAR_SPEED
  cruiseBobPhase += delta * .18
  const cruiseBlend = cruiseBlendStartedAt ? Math.min(1, (now - cruiseBlendStartedAt) / 900) : 1
...
function animate(now = performance.now()) {
  ...
  if (!reducedMotion.value) {
    ...
    const effectPulse = effectAge < effectDuration ? Math.sin(Math.min(1, effectAge / effectDuration) * Math.PI) : 0
    ...
    if (core) {
      ...
      if (moonOrbit) moonOrbit.rotation.y -= delta * .16
    }
    if (sun) {
    ...
    if (blackHole) {
      ...
      if (disk) disk.rotation.z += delta * (.12 + (discoveryEffect === 'black-hole' ? effectPulse * 1.05 : 0))
      const lensedDisk = blackHole.getObjectByName('black-hole-lensed-disk')
      if (lensedDisk) lensedDisk.rotation.z -= delta * (.045 + (discoveryEffect === 'black-hole' ? effectPulse * .32 : 0))
      const halo = blackHole.getObjectByName('black-hole-halo')
      if (halo) halo.scale.setScalar(92 + Math.sin(elapsed * .7) * 3.5 + (discoveryEffect === 'black-hole' ? effectPulse * 22 : 0))
    }
    if (spaceStation) {
      stationOrbitPhase -= delta * Math.PI * 2 / 86
      spaceStation.position.copy(stationOrbitPosition(stationOrbitPhase))
      spaceStation.rotation.set(
        .18 + Math.sin(stationOrbitPhase * 1.7) * .04,
        -stationOrbitPhase + Math.PI / 2,
        -.12 + Math.cos(stationOrbitPhase * 2.1) * .05,
      )
    ...
    if (satellite) {
      if (focusedDiscoveryId !== 'satellite') {
        satelliteOrbitPhase -= delta * .032
        satellite.position.set(Math.cos(satelliteOrbitPhase) * 92, -34 + Math.sin(satelliteOrbitPhase * 4.2) * 4, Math.sin(satelliteOrbitPhase) * 68)
      }
    ...
    if (spacecraft) {
      ...
      if (spacecraftLaunchActive && launchAge >= 0 && launchAge < 4.2) {
        ...
        spacecraft.position.copy(new THREE.QuadraticBezierCurve3(spacecraftLaunchFrom, spacecraftLaunchControl, spacecraftLaunchTo).getPoint(eased))
        ...
        const pathTangent = spacecraftFlightPath.getTangentAt(pathProgress).normalize()
        spacecraft.position.copy(pathPosition)
        spacecraft.rotation.set(
          Math.asin(THREE.MathUtils.clamp(pathTangent.y, -.8, .8)) * .5,
          -Math.atan2(pathTangent.z, pathTangent.x),
          Math.sin(pathProgress * Math.PI * 2) * .12,
        )
        spacecraft.scale.setScalar(lowQuality ? .62 : .8)
      }
      spacecraft.traverse((child) => {
        const boost = discoveryTourId === 'spacecraft' ? 1 : discoveryEffect === 'spacecraft' ? effectPulse : 0
        if (child.name === 'spacecraft-drive-ring') {
          child.scale.setScalar(1 + Math.sin(elapsed * 3.6) * .08 + boost * .38)
        } else if (child.name === 'spacecraft-exhaust') {
          const pulse = 1 + Math.sin(elapsed * 21 + child.position.z) * .08
          ...
          child.scale.set(1 + boost * .32, baseLength * (1 + boost * .62 + Math.sin(elapsed * 18 + child.position.z) * .045), 1 + boost * .32)
          ...
          for (let index = 0; index < positions.count; index++) {
            let x = positions.getX(index) - speeds[index] * delta * (1 + boost * 1.8)
            if (x < -47) x = -10.8
            positions.setX(index, x)
          }
          positions.needsUpdate = true
          points.material.opacity = .48 + boost * .42
        } else if (child.name === 'spacecraft-ion-trails') {
          const material = (child as THREE.LineSegments).material as THREE.LineBasicMaterial
          material.opacity = .045 + boost * .32
        }
      })
      for (const material of enginePlumeMaterials) {
        material.uniforms.uTime.value = elapsed
        material.uniforms.uBoost.value = discoveryTourId === 'spacecraft' ? 1 : discoveryEffect === 'spacecraft' ? effectPulse : 0
      }
    }
    ...
    for (const [id, object] of nodeObjects) {
      ...
      const base = THREE.MathUtils.lerp(currentScale, targetScale, 1 - Math.exp(-delta * 5.2))
      ...
      object.traverse((child) => {
        ...
        if ('opacity' in material && Number.isFinite(targetOpacity)) {
          material.opacity = THREE.MathUtils.lerp(Number(material.opacity), targetOpacity, 1 - Math.exp(-delta * 6))
        }
        const targetGlow = Number(material.userData.targetGlowOpacity)
        if (Number.isFinite(targetGlow) && material.uniforms?.glowOpacity) {
          material.uniforms.glowOpacity.value = THREE.MathUtils.lerp(material.uniforms.glowOpacity.value, targetGlow, 1 - Math.exp(-delta * 6))
        }
      })
    }
    ...
    if (warpMaterial) {
      ...
      const effectWarp = discoveryEffect === 'black-hole'
        ? effectPulse * .18
  ...
  if (blackHole && camera && focusedDiscoveryId !== 'black-hole') {
    const viewOffset = (blackHole.userData.viewOffset as THREE.Vector3).clone()
      ...
      .applyQuaternion(camera.quaternion)
    const anchoredPosition = camera.position.clone().add(viewOffset)
    if (resettingDiscoveryId === 'black-hole' && blackHoleResetActive && cameraFlight) {
      const progress = cameraFlight.duration ? Math.min(1, (now - cameraFlight.startedAt) / cameraFlight.duration) : 1
      const eased = progress * progress * progress * (progress * (progress * 6 - 15) + 10)
      blackHole.position.lerpVectors(blackHoleResetFrom, anchoredPosition, eased)
    } else {
