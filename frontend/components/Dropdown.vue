...
.ui-dropdown__menu { position: absolute; z-index: 100; top: calc(100% + 6px); right: 0; width: 100%; min-width: 100%; max-width: min(280px, calc(100vw - 24px)); max-height: min(280px, calc(100dvh - 110px)); box-sizing: border-box; padding: 5px; overflow-y: auto; border: 1px solid color-mix(in srgb, var(--border) 82%, transparent); border-radius: var(--ui-radius-control, 9px); background: color-mix(in srgb, var(--ld-bg-card) 98%, transparent); box-shadow: 0 16px 36px color-mix(in srgb, var(--ld-shadow) 58%, transparent); backdrop-filter: blur(18px); }
.ui-dropdown__menu button { display: flex; width: 100%; height: 34px; min-height: 34px; box-sizing: border-box; align-items: center; justify-content: space-between; gap: 8px; padding: 0 8px; border: 0; border-radius: 7px; background: transparent; color: var(--c-text-2); cursor: pointer; font: inherit; transition: background .15s, color .15s; }
...
@media (max-width: 640px) { .ui-dropdown { width: min(var(--dropdown-width, 132px), 46vw); } }
