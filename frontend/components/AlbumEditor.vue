<template>
  <div class="album-editor">
    ...
    <section class="album-hero-card">
      <div>
        <span>WIND-BORNE ALBUM</span>
        <h1>{{ form.title || '新建一册时光' }}</h1>
        <p>原图被完整保留，页面使用一份轻量 WebP。照片的时间与地点只有确认后才会公开。</p>
      </div>
      <strong>{{ items.length }}</strong><small>PHOTOS</small>
    </section>
