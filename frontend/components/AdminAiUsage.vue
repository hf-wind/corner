<template>
  <section class="ai-usage-overview" aria-labelledby="ai-usage-title">
    ...
    <div class="admin-table-shell usage-table">
      <a-table :data-source="usage.actors || []" :columns="columns" row-key="actorId" size="small" :pagination="false" :scroll="{ x: 980 }" :locale="{ emptyText: '暂无 AI 使用记录' }">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'calls'">{{ formatNumber(record.calls) }}</template>
...
.usage-table strong,.usage-table small { display:block; }.usage-table small { margin-top:2px; color:var(--c-text-3); font-size:.65rem; }.pricing-note { margin:8px 0 0; color:var(--c-text-3); font-size:.62rem; }
...
@media(max-width:1100px){.usage-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:640px){.usage-head{align-items:flex-start;flex-direction:column}.usage-metrics{grid-template-columns:1fr}}
</style>
