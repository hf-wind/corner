...
export type ThemePalette = 'fresh' | 'ocean'

const PALETTES: ThemePalette[] = ['fresh', 'ocean']
...
function normalizePalette(value: unknown): ThemePalette {
  return value === 'ocean' ? 'ocean' : 'fresh'
}
...
export function useTheme(): ThemeContext {
  ...
  const initialPalette = typeof window === 'undefined' ? 'fresh' : normalizePalette(clientState.get('site', 'palette', 'fresh'))
  ...
  const palette: Ref<ThemePalette> = useSharedState<ThemePalette>('palette', () => initialPalette)
  ...
  function setTheme(mode: string) {
    const next = (['light', 'dark', 'auto'] as ThemeMode[]).includes(mode as ThemeMode) ? mode : 'auto'
  ...
  function init() {
    ...
    theme.value = saved
    palette.value = normalizePalette(clientState.get('site', 'palette', 'fresh'))
    applyPalette()
    applyTheme()
