...
export class AiNativeService {
  ...
  >();
  private readonly styleTimers = new Map<
    string,
    ...
    const [postComments, momentComments] = await Promise.all([
      this.prisma.comment.findMany({
        where: {
          ...
          OR: [{ content: contains }, { authorName: contains }],
        },
    ...
    return [
      ...
      ...moments.map((item) =>
        createCard(
          ...
          item.id,
          item.title,
          `/moments?focus=${encodeURIComponent(item.slug)}`,
          item.content || item.excerpt,
          item.publishedAt,
        ),
      ),
      ...
      ...libraryItems.map((item) =>
        createCard(
          ...
          item.title,
          `/library/${item.slug}`,
          [item.originalTitle, item.creator, item.summary, item.reflection]
            .filter(Boolean)
            .join(' · '),
          item.publishedAt,
        ),
      ...
      ...places.map((item) =>
        createCard(
          ...
          item.name,
          `/places/${item.slug}`,
          [item.address, item.city, item.province, item.country]
            .filter(Boolean)
            .join(' · '),
        ),
      ),
      ...photos.map((item) =>
        createCard(
          ...
          item.id,
          `照片 · ${item.album.title}`,
          `/albums/${item.album.slug}?photo=${item.id}`,
          item.caption,
          null,
      ...
      ...tags.map((item) =>
        createCard(
          ...
          item.id,
          `标签 · ${item.name}`,
          `/tags?tag=${encodeURIComponent(item.slug)}`,
          `${item.name} · ${item._count.posts} 篇文章`,
          null,
          {
            slug: item.slug,
            icon: item.icon,
  ...
  async syncIndex() {
    ...
    for (const node of nodes) {
      ...
      const searchableMetadata = Object.entries(metadata)
        ...
        .filter(Boolean)
        .join(' ');
      const body = [node.title, node.excerpt || '', searchableMetadata]
        .filter(Boolean)
        .join('\n');
  ...
  async search(query: string, types: string[] = [], limit = 6) {
    ...
    const contentResult = items
      .map((item) => ({
        ...
        score: (() => {
          const haystack =
            `${item.title} ${item.excerpt || ''} ${item.body}`.toLocaleLowerCase();
          const exact = haystack.includes(normalizedQuery) ? 0.65 : 0;
          const title = item.title.toLocaleLowerCase().includes(normalizedQuery)
            ? 0.9
            : 0;
          return (
            this.cosine(
    ...
    const [contentResultDirect, commentResult] = await Promise.all([
      this.searchPublishedContent(
        normalizedQuery,
        Math.max(1, Math.min(80, limit * 5)),
      ),
      this.searchCommentCards(
        normalizedQuery,
        Math.max(1, Math.min(80, limit * 4)),
      ),
    ]);
  ...
  async explore(query: string, types: string[] = [], limit = 6) {
    ...
    if (cards.length && (await this.ai.isConfigured())) {
      try {
        const style = await this.ai.getSiteStyleInstruction('内容探索推荐');
  ...
  async insight(contentType: string, slugOrId: string) {
    ...
    if (await this.ai.isConfigured()) {
      try {
        const style = await this.ai.getSiteStyleInstruction('内容洞察');
        const text = await this.ai.chat(
          [
            {
              role: 'system',
              content: [
                '为阅读条生成可核验内容，只依据材料，严格输出 JSON。',
                style,
              ]
                .filter(Boolean)
                .join('\n\n'),
  ...
  async transform(
    ...
    const style = await this.ai.getSiteStyleInstruction('选区改写', userId);
    const instruction = {
      polish: '润色但不改变事实和语气',
      expand: '扩写并补足过渡，但不得新增事实',
      compress: '压缩冗余并保留关键信息',
      style: '参考当前站点文风重新组织并补全表达，保留原意和事实；信息不足时只补自然过渡，不得虚构',
    }[action];
    const output = await this.ai.chat(
      [
        {
          role: 'system',
          content: `你是作者的创作副驾驶。${instruction}。保留 Markdown。\n\n${style}`,
        },
  ...
  async rebuildStyle(userId: string) {
    ...
    const status = await this.ai.getSiteStyleStatus(userId);
    if (!status.eligible) {
      throw new BadRequestException(
        `至少需要 ${status.minimum} 篇已发布文章或瞬间，当前只有 ${status.sampleCount} 篇`,
      );
    }
    const maxSamples = Math.max(8, Math.min(100, cfg.ai_style_max_samples));
    const charLimit = Math.max(
      400,
      Math.min(6000, cfg.ai_style_sample_char_limit),
    );
    ...
    const samples = [...posts, ...moments]
      .sort(
        (left, right) =>
          Number(right.publishedAt || 0) - Number(left.publishedAt || 0),
      )
      .map((item) => {
        const body = this.plain(item.content, charLimit);
        return { body, sample: `${item.title || ''}\n${body}` };
      })
          ...
          [
            {
              role: 'system',
              content:
                '分析作者表达习惯，只总结语气、节奏、措辞、结构和明确应避免的习惯；不要复述或复制样本文句，仅输出 JSON。',
            },
            {
              role: 'user',
    ...
    const saved = await this.prisma.aiAuthorStyleProfile.upsert({
      where: { userId },
  ...
  async scheduleStyleRebuild(userId: string, delayMs = 4_000) {
    const cfg = await this.ai.getConfig();
    if (!cfg.ai_style_enabled || !cfg.ai_style_auto_rebuild) return;
    const current = this.styleTimers.get(userId);
    if (current) clearTimeout(current);
    const timer = setTimeout(() => {
      this.styleTimers.delete(userId);
      void this.rebuildStyle(userId).catch(() => undefined);
    }, delayMs);
    timer.unref?.();
    this.styleTimers.set(userId, timer);
  }
  ...
  async narrative(userId: string, dto: AiNarrativeDto) {
    ...
    if (await this.ai.isConfigured()) {
      try {
        const style = await this.ai.getSiteStyleInstruction('记忆叙事', userId);
  ...
  async privateQuery(userId: string, query: string) {
    ...
    if (matches.length && (await this.ai.isConfigured())) {
      try {
        const style = await this.ai.getSiteStyleInstruction(
          '私有记忆回答',
          userId,
        );
        answer = await this.ai.chat(
  ...
  async usageAnalytics(requestedPage = 1, requestedPageSize = 15) {
    ...
    const pageSize = Math.min(
      50,
      Math.max(10, Math.floor(requestedPageSize) || 15),
    );
    ...
    const pricingTier =
      String(process.env.DEEPSEEK_PRICING_TIER || 'flash').toLowerCase() ===
      'pro'
        ? 'pro'
        : 'flash';
    ...
    const inputPrice = Math.max(
      0,
      Number(process.env.AI_INPUT_PRICE_PER_1M || officialPricing.cacheMiss),
    );
    const outputPrice = Math.max(
      0,
      Number(process.env.AI_OUTPUT_PRICE_PER_1M || officialPricing.output),
    );
    const present = (input = 0, output = 0) => ({
      ...
      estimatedCostCny: Number(
        (
          (input * officialPricing.cacheMiss +
            output * officialPricing.output) /
          1_000_000
        ).toFixed(6),
      ),
      estimatedCostCnyMin: Number(
        (
          (input * officialPricing.cacheHit + output * officialPricing.output) /
          1_000_000
        ).toFixed(6),
      ),
    });
    ...
    const actors = [
      ...userGroups.map((item) => {
        ...
        const profile = item.userId
          ? profileByUser.get(item.userId)
          : undefined;
        const visit = profile
          ? latestVisit.get(profile.visitorIdHash)
          : undefined;
        return {
          actorType: 'user',
          actorId: item.userId,
    ...
    return {
      pricing: {
        ...
        modelTier:
          pricingTier === 'pro' ? 'deepseek-v4-pro' : 'deepseek-v4-flash',
