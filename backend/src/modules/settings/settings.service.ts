...
export class SettingsService {
  ...
  async health() {
    ...
    const checks = [
      ...
      { key: 'geetest_id', label: '极验验证ID', configured: env('GEETEST_CAPTCHA_ID'), required: process.env.NODE_ENV === 'production', source: '服务端环境' },
      { key: 'geetest_key', label: '极验验证KEY', configured: env('GEETEST_CAPTCHA_KEY'), required: process.env.NODE_ENV === 'production', source: '服务端环境' },
