...
type LimitRule = { name: string; limit: number; windowSeconds: number; identity?: 'email' };
...
export class RedisRateLimitGuard implements CanActivate {
  ...
  private async consume(request: Request, response: Response, rule: LimitRule) {
    const identity = rule.identity === 'email'
      ? createHash('sha256').update(String((request.body as any)?.email || '').trim().toLowerCase()).digest('hex')
    ...
    response.setHeader('RateLimit-Remaining', String(Math.max(0, rule.limit - current)));
    response.setHeader('RateLimit-Reset', String(ttl));
    if (current <= rule.limit) return;

    response.setHeader('Retry-After', String(ttl));
    throw new HttpException(`请求过于频繁，请在 ${ttl} 秒后重试`, HttpStatus.TOO_MANY_REQUESTS);
  }

  private rulesFor(request: Request): LimitRule[] {
    const method = request.method.toUpperCase();
    const path = request.path;
    const rules: LimitRule[] = [{ name: 'global', limit: 240, windowSeconds: 60 }];

    if (method === 'POST' && path.endsWith('/auth/login')) {
      rules.push({ name: 'login-ip', limit: 10, windowSeconds: 600 });
      rules.push({ name: 'login-email', limit: 8, windowSeconds: 600, identity: 'email' });
    } else if (method === 'POST' && path.endsWith('/auth/register')) {
      rules.push({ name: 'register-ip', limit: 5, windowSeconds: 3600 });
      rules.push({ name: 'register-email', limit: 3, windowSeconds: 3600, identity: 'email' });
    } else if (method === 'POST' && path.endsWith('/auth/send-code')) {
      rules.push({ name: 'send-code-ip', limit: 8, windowSeconds: 3600 });
      rules.push({ name: 'send-code-email', limit: 5, windowSeconds: 3600, identity: 'email' });
    } else if (method === 'POST' && /\/friend-link\/(?:apply|inspect-site)/.test(path)) {
      rules.push({ name: 'friend-link', limit: 6, windowSeconds: 3600 });
    } else if (method === 'POST' && path.endsWith('/friend-link/remove/send-code')) {
      rules.push({ name: 'friend-remove-code-ip', limit: 8, windowSeconds: 3600 });
      rules.push({ name: 'friend-remove-code-email', limit: 5, windowSeconds: 3600, identity: 'email' });
    } else if (method === 'POST' && path.endsWith('/friend-link/remove/verify')) {
      rules.push({ name: 'friend-remove-verify-ip', limit: 10, windowSeconds: 600 });
      rules.push({ name: 'friend-remove-verify-email', limit: 8, windowSeconds: 600, identity: 'email' });
    } else if (method === 'POST' && /\/(?:comments|moment-comments)(?:\/[^/]+\/like)?$/.test(path)) {
    ...
    } else if (method === 'POST' && /\/media\/(?:upload|import-url)$/.test(path)) {
      rules.push({ name: 'media-upload', limit: 30, windowSeconds: 3600 });
    } else if (method === 'GET' && path.endsWith('/posts') && request.query.search) {
