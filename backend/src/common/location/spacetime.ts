...
import type { LocationPrecision, LocationSource, LocationVisibility } from './public-location';
...
export async function prepareSpacetime(
  ...
  const visibility = locationVisibility(dto.locationVisibility ?? existing?.locationVisibility);
  const precision = locationPrecision(dto.locationPrecision ?? existing?.locationPrecision);
  if (placeId) await assertPlace(placeId);
  if (!placeId && visibility !== 'private') throw new BadRequestException('公开位置前必须先选择地点');
  if (visibility === 'public' && precision !== 'exact') throw new BadRequestException('精确公开必须使用精确坐标');
  if (visibility === 'blurred' && precision === 'exact') throw new BadRequestException('模糊公开不能使用精确坐标');
